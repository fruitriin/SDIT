...
複数のサブエージェントで並列に実装を行う場合、**git worktree を使ってリポジトリを分離する**。

**ルール:**
- 同一ファイルを変更する可能性があるタスクは、必ず worktree を分離する
- 特に `main.rs` のような中心ファイルの変更は **1エージェントに集約** するか、worktree で分離する
- worktree 分離しない並列実行は、変更対象ファイルが完全に独立している場合のみ許可する

**背景:**
Phase 2.5 で3エージェント並行実装を行った際、`main.rs` の変更が競合し、
片方が書いた変更をもう片方が上書きする問題が発生した。
worktree を使えば各エージェントが独立したワーキングツリーで作業でき、

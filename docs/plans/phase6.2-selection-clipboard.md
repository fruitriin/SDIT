...
- クリップボード上限 1 MiB で制限
- **M-4（修正済み）**: BRACKETED_PASTE バイパス — クリップボード内容に `\x1b[200~`/`\x1b[201~` が含まれる場合、ペースト終端を偽装してシェルコマンド実行が可能（Terminal Injection via Clipboard）。サニタイズ処理を追加して対策。
- **L-1**: OSC 52 サイズ超過時のサイレント破棄（ログなし）
- **L-3**: decode_base64 に入力長制限なし（vte のバッファ制限に依存）
- **L-5**: ペースト側にサイズ制限なし（巨大クリップボードで PTY write channel 消費の可能性）

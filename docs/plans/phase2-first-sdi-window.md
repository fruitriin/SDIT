...
- [x] PTYとウィンドウを接続 ← **Step 4 完了**
- [x] キー入力をPTYに送信 ← **Step 5 完了**
...
### セキュリティレビュー結果
- **Low（修正済み）**: `vertex_buffer` サイズが 80×24 固定 → Step 4 で `ensure_capacity()` を追加し動的リサイズ対応済み
- `unsafe_code = "deny"` 維持確認済み
- bytemuck は Pod/Zeroable derive マクロで安全に使用
- ペネトレーションテスト: Step 4（PTY接続）まで不要

## Step 4+5 実装記録（2026-03-11）

### 実装内容

**`crates/sdit/src/main.rs`**（大幅書き換え）:
- `SditEvent` を `PtyOutput` / `ChildExit(i32)` に更新
- `TerminalState { terminal, processor }` を `Arc<Mutex<>>` で包み、メインスレッドと PTY リーダースレッドで共有
- `SditApp` に `term_state`, `pty_write_tx`, `pty_thread`, `event_proxy` を追加
- `init()`: ウィンドウサイズ + フォントメトリクスから `cols/rows` を計算、`Terminal::new()` + `Pty::spawn()` を呼び出し
- `spawn_pty_reader()`: `pty-reader` スレッドを生成。PTY 読み取り → `Processor::advance()` → `PtyOutput` イベント送信
- `redraw_from_terminal()`: `term_state` ロック → `grid()` 取得 → `update_from_grid()` → GPU バッファ更新
- `handle_resize()`: GPU サーフェス + `Terminal::resize()` を連携
- `key_to_bytes()`: `TermMode::APP_CURSOR` 対応 Arrow キー変換 + Ctrl+a-z 変換
- `ApplicationHandler::window_event(KeyboardInput)`: `pty_write_tx.try_send()` でキー送信
- `ApplicationHandler::user_event(PtyOutput)`: `redraw_from_terminal()` → `request_redraw()`
- `ApplicationHandler::user_event(ChildExit)`: `event_loop.exit()`
- `about_to_wait`: 連続描画を停止、イベント駆動に変更

**`crates/sdit-render/src/pipeline.rs`**（変更）:
- `CellPipeline::new()` に `initial_capacity: usize` 引数を追加し動的バッファ確保
- `CellPipeline::ensure_capacity()` を追加（必要セル数 > 確保容量なら 2 倍に再確保）
- `vertex_buffer_capacity` フィールドを追加して現在の容量を追跡

### 最終セキュリティレビュー修正（2026-03-11）

**Medium 2件を修正:**
1. `atlas::write()` の境界チェック: `debug_assert` → release でも有効な `if` ガード + `log::error!` + 早期 return
2. `CellPipeline::ensure_capacity()` の整数オーバーフロー: `needed * 2` → `saturating_mul(2)` + 64MB 上限

**Low 5件（未対応 — Phase 3 以降で対応）:**
- ~~`try_send` 失敗時のサイレントドロップ → ログ追加を検討~~ **修正済み（PTY read/write スレッド分離時に対応）**
- 高速PTY出力時の PtyOutput イベント連射 → バッチング導入を検討
- Ctrl+記号キーの不完全マッピング → Phase 5 で完全キーマップ実装
- Mutex Poisoning 後のデータ破損 → 安全なリセット or 終了を検討 (SV-6)
- アトラス満杯時のグリフ欠落がサイレント → 動的拡張を将来実装
- `update_from_grid` の容量チェック不在 → 内部 assert 追加を検討

**PTY read/write デッドロック修正（2026-03-12）:**
- **根本原因**: `spawn_pty_reader` が1スレッドで read と write を処理。`pty.read()` がブロッキングのため `pty_write_rx.try_recv()` に到達せず、キー入力が PTY に届かなかった
- **修正**: `Pty::try_clone_writer()` で master fd をクローンし、read/write を独立スレッドに分離
- セキュリティレビュー SV-2（try_send サイレント破棄）は `log::warn!` 追加で修正
- セキュリティレビュー SV-4（ライタースレッド異常終了）は `ChildExit` 通知追加で修正
- 残り Low: SV-1（入力長制限）、SV-3（PtyWriter newtype）、SV-5（リサイズ未実装）、SV-6（Mutex poison）

### セキュリティレビュー結果（Step 4+5）
- `mpsc::sync_channel(64)` による書き込みキューの容量制限（DoS 対策）
- `Mutex::lock().unwrap_or_else(PoisonError::into_inner)` で PoisonError を安全にハンドリング
- PTY リーダーバッファ: 8192 バイト固定（スタックアロケーション、ヒープ攻撃なし）
- EIO (`raw_os_error() == Some(5)`) の明示的ハンドリングでゾンビプロセス回避
- PTY リサイズはウィンドウリサイズ時に `Terminal::resize()` のみ実行（PTY の SIGWINCH は未実装 — 機能上の制限、セキュリティ問題なし）
- `unsafe_code = "deny"` 維持確認済み

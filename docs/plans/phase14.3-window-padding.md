...
| ID | 重要度 | 内容 | 対応 |
|---|---|---|---|
| L-1 | Low | Uniforms アライメントのコンパイル時アサート不在 | 将来フィールド追加時に検討 |
| I-1 | Info | パディング領域クリックは col=0/row=0 として処理される | 既存の上限チェックで問題なし |
| I-2 | Info | Hot Reload 後の再描画タイミング | 次のウィンドウイベントで反映される |

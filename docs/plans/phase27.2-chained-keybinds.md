...
- `KeyBinding.action_chain: Vec<Action>` を追加（デフォルト: 空）
- validate() で chain 長を 16 に制限（DoS 防止）
- `dispatch_keybind()` で primary + chain を順番に実行
- `try_dispatch_keybind()` で resolve + dispatch を統合
- event_loop.rs: `try_dispatch_keybind` 呼び出し1行に簡略化
- テスト: 448 件 PASS

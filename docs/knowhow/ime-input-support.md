...
**重要**: 単純な `replace` は **ダブルリプレースバイパス** に脆弱。例: `"\x1b[200\x1b[200~~"` → 1回目の replace で内側の `\x1b[200~` が消えると外側の `\x1b[200~` が再合成される。対策として収束するまでループする:
...
1文字の場合はブラケット不要（通常の文字入力と同じ）。判定は `text.chars().count() > 1`（`text.len()` はバイト数なので不正確）。

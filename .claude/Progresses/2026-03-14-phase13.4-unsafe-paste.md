...
## タスク: Phase 13.4 — Unsafe Paste 警告

### 設定追加
- [x] PasteConfig 構造体追加（confirm_multiline: bool, default true）
- [x] Config に paste フィールド追加
- [x] save_with_comments に [paste] セクションコメント追加

### Unsafe Paste 検出・確認
- [x] is_unsafe_paste: 改行 + bracketed paste 脱出シーケンス検出
- [x] confirm_unsafe_paste: rfd で警告ダイアログ + プレビュー（制御文字サニタイズ済）
- [x] event_loop.rs の Action::Paste に組み込み

### セキュリティ修正
- [x] M-1: bracketed paste 脱出インジェクション検出（\x1b[201~ チェック追加）
- [x] L-1: プレビュー制御文字サニタイズ

### テスト
- [x] PasteConfig serde テスト（2件）
- [x] is_unsafe_paste テスト（3件: 改行検出、bracketed安全、脱出インジェクション検出）

### 品質ゲート
- [x] Stage 1: cargo fmt/clippy/test — 259テスト全パス
- [x] Stage 2: security-review 完了（M-1/L-1修正済み、L-2/L-3/I-1/I-2記録済み）

...
## タスク: Phase 10.3a — カラー絵文字対応（Atlas RGBA 化）

### 実装
- [x] Step 1: Atlas テクスチャフォーマットを R8Unorm → Rgba8Unorm に変更 — data を 4bytes/pixel に、write() を RGBA 対応に
- [x] Step 2: font.rs のグレースケール → RGBA 昇格 — Content::Mask のデータを R=G=B=255, A=alpha に変換
- [x] Step 3: font.rs で Content::ColorBitmap 処理 — swash の RGBA ビットマップをそのままアトラスに格納
- [x] Step 4: GlyphEntry に is_color フラグ追加 — カラーグリフかどうかをパイプラインに伝える
- [x] Step 5: CellVertex に is_color_glyph フラグ追加 + シェーダー修正 — カラーグリフは fg 色の代わりにテクスチャ色を使う
- [x] Step 6: テスト追加 — Atlas RGBA write、グレースケール昇格、ColorBitmap テスト

### 品質ゲート
- [x] Stage 1: `cargo fmt --check && cargo clippy --all-targets && cargo test`（全 171 + 4 = 175 テスト通過）

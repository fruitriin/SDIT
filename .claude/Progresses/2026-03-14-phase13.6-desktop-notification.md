...
- [x] OSC 99 パース（Kitty 互換: 簡易実装）
- [x] take_notification() メソッド追加

### イベント・通知 (sdit)
- [x] SditEvent::DesktopNotification 追加
- [x] PTY リーダーで take_notification → イベント送出
- [x] notify-rust クレートでシステム通知発行（別スレッド非同期）

### セキュリティ修正
- [x] M-1: 通知テキスト制御文字サニタイズ（sanitize_notification_text）
- [x] M-2: 通知スレッドリーク防止（AtomicBool in-flight フラグ）
- [x] L-1: UTF-8 バイト境界安全トランケーション（truncate_utf8）
...
### テスト・品質ゲート
- [x] OSC 9/99 パーステスト 7件 + NotificationConfig serde 2件
- [x] Stage 1: 288テスト全パス
- [x] Stage 2: security-review 完了（M-1/M-2/L-1修正済み、L-2/L-3/I-1記録済み）

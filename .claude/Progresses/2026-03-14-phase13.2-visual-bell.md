...
## タスク: Phase 13.2 — ビジュアルベル + システム通知

### 設定・イベント基盤
- [x] BellConfig 設定項目追加（bell.visual, bell.dock_bounce, bell.duration_ms）
- [x] SditEvent::BellRing(SessionId) 追加
- [x] window.rs: BEL 検出時に SditEvent::BellRing 送信

### VisualBell 実装
- [x] VisualBell 構造体（ring/intensity/completed メソッド）
- [x] WindowState に VisualBell を保持

### レンダリング
- [x] render.rs: ベルオーバーレイ描画（背景色と白を intensity で補間、フェードアニメーション）
- [x] フェード中の継続再描画（request_redraw）

### macOS Dock バウンス
- [x] winit Window::request_user_attention(Informational) 使用（unsafe 不要）

### Hot Reload・テスト
- [x] bell 設定の Hot Reload 対応
- [x] ユニットテスト（BellConfig serde 3件, VisualBell 5件）

### セキュリティ修正
- [x] M-1: BEL bomb レート制限（ring() でアニメーション中は新規リング無視）
- [x] L-1: duration_ms = 0 の NaN ガード（intensity_inner に is_zero チェック）
- [x] L-2: duration_ms 上限なし → clamped_duration_ms() で 1..5000 にクランプ

### 品質ゲート
- [x] Stage 1: cargo fmt/clippy/test — 184テスト全パス
- [x] Stage 1（セキュリティ修正後再実行）: 全パス
- [x] Stage 2: security-review 完了（M-1修正済み、L-1/L-2修正済み、I-1/I-2記録済み）

...
## タスク: Phase 13.3 — 背景透過 + macOS blur

### 設定追加
- [x] WindowConfig 構造体追加（opacity: f32, blur: bool, clamped_opacity()）
- [x] Config に window フィールド追加
- [x] save_with_comments に [window] セクションコメント追加

### ウィンドウ透過
- [x] create_window: with_transparent() + with_blur() 設定
- [x] detach_session_to_new_window: 同様の設定
- [x] 透過条件: opacity < 1.0 || blur の場合のみ with_transparent(true)

### レンダリング
- [x] clear_color の alpha に opacity を反映
- [x] ベルオーバーレイとの統合（bell intensity + opacity）

### Hot Reload
- [x] opacity/blur 変更時のウィンドウ更新（set_blur + request_redraw）

### テスト
- [x] WindowConfig デフォルト・serde テスト（3件）
- [x] opacity クランプテスト（通常 + NaN/Inf）

### セキュリティ修正
- [x] M-1: opacity NaN/Inf ガード（is_finite チェック）
- [x] M-2: BEL ボム PTY リーダー側レート制限（100ms）
- [x] L-2: opacity=1.0 で透過フラグ無効化

### 品質ゲート
- [x] Stage 1: cargo fmt/clippy/test — 252テスト全パス
- [x] Stage 1（セキュリティ修正後再実行）: 全パス
- [x] Stage 2: security-review 完了（M-1/M-2/L-2修正済み、L-1/L-3/I-1記録済み）

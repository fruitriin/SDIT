...
## タスク: Phase 9.1 — スクロールバック内検索

### 実装
- [x] Step 1: sdit-core/terminal/search.rs — 検索エンジン（Grid 走査 + パターンマッチ）
- [x] Step 2: input.rs — 検索ショートカット追加（Cmd+F, Escape, Enter, Shift+Enter）
- [x] Step 3: app.rs — SearchState 構造体 + SditApp フィールド追加
- [x] Step 4: pipeline.rs — update_from_grid にマッチハイライト追加
- [x] Step 5: render.rs — 検索バーUI描画（build_search_bar_cells）
- [x] Step 6: event_loop.rs — 検索モードハンドラ統合（キー入力キャプチャ + IME対応）
- [ ] Step 7: テスト追加

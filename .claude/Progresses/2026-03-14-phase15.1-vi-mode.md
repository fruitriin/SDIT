...
## タスク: Phase 15.1 — vi モード（コピーモード）

### 実装
- [x] vi_mode.rs 新規作成（sdit-core: ViCursor, ViMotion, motion 関数群）
- [x] ViModeState 追加（sdit/app.rs: カーソル位置・選択範囲・モード管理）
- [x] vi モーション実装（h/j/k/l/w/b/e/0/$/{/}/gg/G/H/M/L）
- [x] 選択モード実装（v/V/Ctrl+v → ViSelectionKind）
- [x] ヤンク実装（y → クリップボードへコピー、Esc で vi モード終了）
- [x] 検索統合（/ ? n N → 既存 SearchState 連携）
- [x] vi カーソル描画（render.rs: ブロックカーソル表示）
- [x] event_loop.rs 統合（ToggleViMode アクション、vi モード中のキー処理）
- [x] keybinds.rs に Action::ToggleViMode + デフォルトバインド追加
- [x] テスト（12件: モーション 8件 + 選択/ヤンクは GUI 依存のため unit テスト対象外）

### 品質ゲート
- [x] Stage 1: `cargo fmt --check && cargo clippy --all-targets && cargo test` (261テスト全パス)
- [x] Stage 2: security-review (Medium M-1, M-2 修正済み) + integration-test (シナリオ 027 作成済み)
- [x] 完了処理

...
## タスク: Phase 9.2 — キーバインドカスタマイズ

### 実装
- [x] Step 1: `sdit-core/src/config/keybinds.rs` — Action enum + KeyBinding struct + デフォルトバインド + TOML デシリアライズ
- [x] Step 2: `sdit-core/src/config/mod.rs` — Config に `keybinds` フィールド追加
- [x] Step 3: `sdit/src/input.rs` — `resolve_action()` 関数追加（key+mods → Action ルックアップ）
- [x] Step 4: `sdit/src/event_loop.rs` — Action-based dispatch にリファクタリング
- [x] Step 5: テスト（keybinds デシリアライズ + resolve_action + デフォルトバインド検証）

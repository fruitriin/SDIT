...
## タスク: Phase 21.1+21.2+21.3 バグ修正バッチ + ビルド修正

### Phase 21.1: 新ウィンドウ空白表示の修正
- [x] `window_ops.rs` の `create_window_with_cwd` で新ウィンドウに `focus_window()` を呼ぶ

### Phase 21.2: シングルクリック時のキャレットマーカー修正
- [x] `event_loop.rs` のマウスリリース処理で、ドラッグなし（start == end）の場合 `selection` を `None` にクリア

### Phase 21.3: SGR 7 (INVERSE) 描画実装
- [x] `pipeline.rs` の `update_from_grid` で `CellFlags::INVERSE` チェックを追加し fg/bg 反転

### ビルド修正（前セッション未コミット分）
- [x] `SessionRestoreInfo`, `WindowSnapshot`, `window_sessions` を persistence.rs に追加
- [x] `ToggleCommandPalette` バリアントと `all_with_names()` を keybinds.rs に追加

### 品質ゲート
- [x] Stage 1: `cargo fmt --check && cargo clippy --all-targets && cargo test`
- [x] Stage 2: セキュリティレビュー（問題なし）

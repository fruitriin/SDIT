...
- [x] `Session` 型実装
- [x] `SessionManager` 型実装

### 3.2 main.rs 複数ウィンドウ対応リファクタ
- [x] `WindowState` 型 + `SditApp` を `HashMap<WindowId, WindowState>` ベースに変更
- [x] `window_event()` で WindowId ベース処理
- [x] `user_event(PtyOutput)` に SessionId を含めて正しいウィンドウを再描画

### 3.3 新規ウィンドウ生成
- [x] Cmd+N / Ctrl+Shift+N で新規ウィンドウ生成

### 3.4 ウィンドウ・セッション終了
- [x] CloseRequested → そのウィンドウのみ終了
- [x] ChildExit → 対応ウィンドウ閉じ
- [x] 全ウィンドウ閉じ → app 終了

### 3.5 完了処理
- [x] `cargo test` + `scripts/check.sh` 全通過
- [x] セキュリティレビュー
- [x] Plan 反映・Feedback 更新・Progress アーカイブ・コミット

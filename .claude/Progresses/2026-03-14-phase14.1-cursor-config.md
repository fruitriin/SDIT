...
## タスク: Phase 14.1 — カーソル設定

### 実装
- [ ] CursorConfig 追加（config/mod.rs: style, blinking, color）
- [ ] Terminal 初期化で CursorConfig を反映
- [ ] DECSCUSR 0 でデフォルトスタイルに復帰
- [ ] カーソル色描画対応（render.rs）
- [ ] Hot Reload 対応
- [ ] テスト（CursorConfig serde 3件 + DECSCUSR 0 リセット 1件）

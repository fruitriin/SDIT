...
## タスク: Phase 13.5 — Kitty Keyboard Protocol

### Terminal 状態管理 (sdit-core)
- [x] KittyKeyboardFlags 構造体（5ビットフラグ）
- [x] KittyFlagStack（8エントリ固定サイズスタック、push/pop/set/current）
- [x] Terminal に kitty_flags フィールド追加
- [x] CSI ハンドラ: `CSI > flags u` (push), `CSI < n u` (pop), `CSI ? u` (query)

### キーエンコーディング (sdit)
- [x] Kitty キーマッピングテーブル + CSI u エンコーディング関数
- [x] key_to_bytes を Kitty/レガシーディスパッチに改修
- [x] 修飾キー付きキー報告（Shift+Arrow 等）

### セキュリティ修正
- [x] M-1: CSI push パラメータ u16→u8 キャスト（& 0x1f でクランプ）
- [x] M-2: CSI pop パラメータ上限（clamp(1, 8)）
- [x] I-1: FlagStack オーバーフロー（サイレント失敗に変更）

### テスト
- [x] KittyFlagStack テスト（6件）
- [x] CSI u エンコーディングテスト（12件）
- [x] CSI ハンドラテスト（2件）

### 品質ゲート
- [x] Stage 1: cargo fmt/clippy/test — 279テスト全パス
- [x] Stage 2: security-review 完了（M-1/M-2/I-1修正済み、L-1/L-2/L-3記録済み）

...
- ~~ノウハウの読み込み順のブラッシュアップしたい~~ → **対応済み**: CLAUDE.md ブートシーケンスを knowhow サブエージェントフィルタリング方式に変更
- ~~統合テストでターミナルに出力しているログをチェックするシナリオかステップを追加したい~~ → **対応済み**: `smoke_headless.rs` と `smoke_gui.rs` に `RUST_LOG=info` + 期待ログメッセージの存在確認を追加

...
## タスク: Phase 15.4 — カスタムリンク + セッションリネーム

### 実装
- [x] カスタムリンク設定（config/mod.rs）
- [x] カスタムリンク検出（terminal/url_detector.rs）
- [x] カスタムリンクアクション（event_loop.rs）
- [x] セッションリネーム（サイドバーダブルクリック→編集）
- [x] セッション名永続化（session.toml）
- [x] テスト

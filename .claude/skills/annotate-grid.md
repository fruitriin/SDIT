...
1. `tools/test-utils/annotate-grid` がビルド済みか確認する
...
4. コマンドを組み立てて実行する:
   ```bash
   ./tools/test-utils/annotate-grid <input> <output> <options>
   ```
...
5. 結果を報告する:
   ...
   - 次のステップとして案内する:
     ```
     注目セルが確認できたら:
       --grid-cell col row N でクリップ: /clip-image
       --rect x y w h でクリップ: /clip-image
     ```
